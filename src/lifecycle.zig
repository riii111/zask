...
test "docker start is a no-op when compose services already run" {
    const json =
        \\{
        \\  "project": {"name":"demo","root":"/tmp/demo","session_name":"demo"},
        \\  "docker": {"enabled": true},
    ...
    try std.testing.expectEqualStrings("ps", recorder.commands.items[1].argv[4]);
}
